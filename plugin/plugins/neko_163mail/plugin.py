...
v0.1: 基于 neko_mail 适配网易163邮箱，支持附件发送
"""
...
class Neko163MailPlugin:
    ...
    def send(
        ...
        """
        try:
            success = self.client.send(to=to, subject=subject, body=body, cc=cc, attachments=attachments)
            if success:
                self.op_log.log_operation(
                    operation_type="send_email",
                    description=f"发送邮件给 {to}" + (f"（含 {len(attachments)} 个附件）" if attachments else ""),
    ...
    def reply(
        ...
        """回复邮件"""
        try:
            success = self.client.reply(
                uid=uid, body=body, folder=folder,
                reply_all=reply_all, attachments=attachments
            )
            if success:
                self.op_log.log_operation(
                    operation_type="reply_email",
                    description=f"回复邮件 {uid}" + ("（回复所有人）" if reply_all else ""),
                    details={"uid": uid, "reply_all": reply_all, "has_attachments": bool(attachments)}
                )
            return {"success": success, "uid": uid}
        except Exception as e:
            return {"error": str(e)}
    ...
    def forward(
        ...
        """转发邮件"""
        try:
            success = self.client.forward(
                uid=uid, to=to, body=body, folder=folder,
                include_attachments=include_attachments
            )
            if success:
                self.op_log.log_operation(
                    operation_type="forward_email",
                    description=f"转发邮件 {uid} 给 {to}",
                    details={"uid": uid, "to": to, "include_attachments": include_attachments}
                )
            return {"success": success, "uid": uid, "to": to}
        except Exception as e:
            return {"error": str(e)}
    ...
    def download_attachment(
        ...
        try:
            result = self.client.download_attachment(
                uid=uid, attachment_index=attachment_index,
                output_dir=output_dir, folder=folder
            )
            if result.get("success"):
                self.op_log.log_operation(
                    operation_type="download_attachment",
                    description=f"下载附件: {result.get('filename', '未知')}",
                    details={"uid": uid, "filename": result.get("filename"), "path": result.get("path")}
                )
            return result
        except Exception as e:
            return {"error": str(e)}
    ...
    def _polling_worker(self):
        """轮询工作线程"""
        import logging
        logger = logging.getLogger("neko_163mail.polling")
