...
- 提示包签名用于检测 instructions 文件 / 模型 / 模式变化
- 线程安全由 asyncio.Lock 保证（插件运行在单事件循环）
- 与 Codex 适配器的 SessionManager 结构对齐，但签名计算
  基于 Cursor 特有的字段（instructions_file_path / model / mode）
"""
...
def compute_prompt_signature(
    *,
    ...
    签名基于影响 Cursor CLI 上下文构建的"环境"因素：
    - instructions 文件路径
    - 默认模型
    - 执行模式
    - 额外标记
...
class SessionManager:
    ...
    用法::
        ...
        # 执行后更新
        if new_session_id:
            await mgr.upsert(new_session_id, cwd, signature)
        elif record:
    ...
    STORE_KEY_PREFIX = "cursor_session:"
    ...
    async def find_resumable(
        ...
        async with self._lock:
            ...
            if record.prompt_signature != prompt_signature:
                return None
            # 不可恢复的错误类别
            if record.last_error in ("unknown_session", "auth_error"):
                return None
            return record
